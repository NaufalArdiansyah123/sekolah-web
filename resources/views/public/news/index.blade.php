                                ...
                                <div class="row g-0">
                                    <div class="col-md-5">
                                        @if($featured->featured_image)
                                            <img src="{{ asset('storage/' . $featured->featured_image) }}" class="img-fluid h-100" alt="{{ $featured->title }}">
                                        @else
                                    ...
                                    <div class="col-md-7">
                                        <div class="card-body p-4">
                                            ...
                                            <p class="card-text text-muted">{{ Str::limit(strip_tags($featured->content), 200) }}</p>
                                            <div class="d-flex justify-content-between align-items-center mt-4">
                                                ...
                                                <a href="{{ route('news.detail', $featured->id) }}" class="btn btn-primary btn-sm">Baca Selengkapnya</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @foreach($blogs->skip(1) as $blog)
                                <div class="col-md-6">
                                    <article class="blog-card shadow h-100">
                                        ...
                                        @else
                                            <img src="https://picsum.photos/300/200?random={{ $loop->index + 2 }}" class="card-img-top" alt="{{ $blog->title }}">
                                        @endif
                                        <div class="card-body p-4">
                                            @php
                                                $categoryBadges = [
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mt-auto">
                                                ...
                                                <a href="{{ route('news.detail', $blog->id) }}" class="btn btn-outline-primary btn-sm">Baca</a>
                                            </div>
                                        </div>
                                    </article>
                                </div>
                    ...
                    <div class="sidebar-card mb-4">
                        ...
                        <div class="card-body">
                            <div class="d-flex flex-column" id="categories">
                                <a href="{{ route('news.index') }}" class="category-item bg-light text-dark {{ !request('category') ? 'bg-primary text-white' : '' }}">
                                    <span>Semua Berita</span>
                                    <span class="badge bg-primary">{{ $blogs->total() }}</span>
                                </a>
                                @foreach($categories as $category)
                                    <a href="{{ route('news.index', ['category' => $category]) }}" class="category-item bg-light text-dark {{ request('category') == $category ? 'bg-primary text-white' : '' }}">
                                        <span>{{ ucfirst($category) }}</span>
                                        <span class="badge bg-primary">{{ \App\Models\BlogPost::published()->where('category', $category)->count() }}</span>
                                    </a>
                                @endforeach
                            </div>
                        </div>
