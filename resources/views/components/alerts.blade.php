...
@if (session('error'))
    <div class="mb-4 bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-200 px-4 py-3 rounded relative" role="alert">
        ...
        @if (session('upload_error_details'))
            <div class="mt-3 p-3 bg-red-50 dark:bg-red-800 border border-red-200 dark:border-red-700 rounded">
                ...
                <ul class="text-xs space-y-1">
                    <li><strong>Max POST Size:</strong> {{ session('upload_error_details.max_post_size') }} ({{ session('upload_error_details.max_post_size_mb') }}MB)</li>
                    <li><strong>Max Upload Size:</strong> {{ session('upload_error_details.max_upload_size') }} ({{ session('upload_error_details.max_upload_size_mb') }}MB)</li>
                </ul>
                <div class="mt-2">
                    <a href="{{ route('admin.videos.troubleshoot') }}" class="text-red-600 dark:text-red-400 underline text-xs">
...
@if (request()->header('Content-Length') && request()->header('Content-Length') > (int)ini_get('post_max_size') * 1024 * 1024)
        ...
        <div class="flex items-start">
            ...
            <div class="ml-3">
                ...
                <div class="mt-2 text-sm">
                    ...
                    <div class="mt-3">
                        <a href="{{ route('admin.videos.troubleshoot') }}" 
