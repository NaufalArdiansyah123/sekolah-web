...
@section('title', 'Statistik Absensi QR Code')
...
<div class="container mx-auto px-6 py-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 mb-8 overflow-hidden">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('admin.qr-attendance.statistics') }}">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <a href="{{ route('admin.qr-attendance.statistics') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg transition-colors duration-200 flex items-center">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden transform hover:scale-105 transition-transform duration-200">
            <div class="bg-gradient-to-r from-yellow-500 to-orange-500 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold">{{ $monthlyStats['terlambat'] ?? 0 }}</p>
            ...
            <div class="p-4">
                <div class="flex items-center justify-between">
                    ...
                    @endphp
                    <span class="text-sm font-semibold text-yellow-600 dark:text-yellow-400">{{ number_format($terlambatPercentage, 1) }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                    <div class="bg-yellow-500 h-2 rounded-full transition-all duration-500" style="width: {{ $terlambatPercentage }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden transform hover:scale-105 transition-transform duration-200">
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold">{{ ($monthlyStats['izin'] ?? 0) + ($monthlyStats['sakit'] ?? 0) }}</p>
            ...
            <div class="p-4">
                <div class="flex items-center justify-between">
                    ...
                    @endphp
                    <span class="text-sm font-semibold text-blue-600 dark:text-blue-400">{{ number_format($izinSakitPercentage, 1) }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                    <div class="bg-blue-500 h-2 rounded-full transition-all duration-500" style="width: {{ $izinSakitPercentage }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden transform hover:scale-105 transition-transform duration-200">
            <div class="bg-gradient-to-r from-red-500 to-red-600 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold">{{ $monthlyStats['alpha'] ?? 0 }}</p>
            ...
            <div class="p-4">
                <div class="flex items-center justify-between">
                    ...
                    @endphp
                    <span class="text-sm font-semibold text-red-600 dark:text-red-400">{{ number_format($alphaPercentage, 1) }}%</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                    <div class="bg-red-500 h-2 rounded-full transition-all duration-500" style="width: {{ $alphaPercentage }}%"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($classStats as $class => $stats)
                            @php
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $class }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $totalAttendanceClass }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($dailyStats as $date => $stats)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $dayNameIndo }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs font-medium rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200">
                                        {{ $hadirCount }}
                                    </span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">{{ $dailyTotal }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
...
const dailyChart = new Chart(dailyCtx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: [
                @foreach($dailyStats as $date => $stats)
                    {{ $stats->where('status', 'hadir')->first()->count ?? 0 }},
                @endforeach
            ],
            borderColor: '#10b981',
            ...
            data: [
                @foreach($dailyStats as $date => $stats)
                    {{ $stats->where('status', 'terlambat')->first()->count ?? 0 }},
                @endforeach
            ],
            borderColor: '#f59e0b',
            ...
            data: [
                @foreach($dailyStats as $date => $stats)
                    {{ $stats->where('status', 'alpha')->first()->count ?? 0 }},
                @endforeach
            ],
            borderColor: '#ef4444',
...
function exportStatistics() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.open(`{{ route('admin.qr-attendance.statistics') }}?month=${month}&year=${year}&export=pdf`, '_blank');
            ...
            window.open(`{{ route('admin.qr-attendance.statistics') }}?month=${month}&year=${year}&export=excel`, '_blank');
