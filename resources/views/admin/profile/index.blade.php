...
<style>
/* Profile Page Styles - Dark Mode Compatible */
.profile-container {
    max-width: 1200px;
    margin: 0 auto;
}
...
<div class="profile-container">
        ...
        <div class="profile-header-content">
            <div class="profile-avatar-section">
                <img src="{{ auth()->user()->avatar_url }}" 
                     ...
                     onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&color=7F9CF5&background=EBF4FF&size=120'">
            ...
            <div class="profile-info">
                ...
                <div class="profile-stats">
                    ...
                    <div class="profile-stat">
                        <div class="profile-stat-value">{{ \App\Models\User::where('created_at', '>=', now()->subDays(30))->count() }}</div>
                        <div class="profile-stat-label">New Users</div>
                    </div>
                    <div class="profile-stat">
                        <div class="profile-stat-value">{{ \App\Models\AttendanceLog::whereDate('attendance_date', today())->count() }}</div>
                        <div class="profile-stat-label">Today's Attendance</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="profile-card">
        ...
        <div class="profile-activity-item">
            <div class="profile-activity-icon update">
                <i class="fas fa-edit"></i>
            </div>
            <div class="profile-activity-content">
                <div class="profile-activity-title">Updated QR attendance settings</div>
                <div class="profile-activity-time">{{ now()->subHours(2)->format('M d, Y \a\t H:i') }}</div>
            </div>
        </div>
        
        <div class="profile-activity-item">
            <div class="profile-activity-icon security">
                <i class="fas fa-shield-alt"></i>
            </div>
            <div class="profile-activity-content">
                <div class="profile-activity-title">Security settings reviewed</div>
                <div class="profile-activity-time">{{ now()->subDays(1)->format('M d, Y \a\t H:i') }}</div>
            </div>
        </div>
        
        <div style="text-align: center; margin-top: 1.5rem;">
            <a href="#" class="profile-btn profile-btn-secondary">
                <i class="fas fa-history"></i>
                View All Activity
            </a>
        </div>
        ...
        if (data.success) {
            Swal.fire({
                ...
                if (typeof updateAllAvatars === 'function') {
                    ...
                    if (navbarAvatar) navbarAvatar.src = data.avatar_url;
...
function generateAvatar() {
    ...
    const colors = ['7F9CF5', '10B981', 'F59E0B', 'EF4444', '8B5CF6', 'F97316'];
...
function logoutAllDevices() {
    ...
    fetch('{{ route("admin.profile.logout-devices") }}', {
        method: 'POST',
        headers: {
