{{-- resources/views/admin/posts/slideshow/edit.blade.php --}}
    ...
    <div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit Slideshow</h1>
                    <p class="text-gray-600 mt-2">Perbarui slideshow untuk homepage</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('admin.posts.slideshow.update', $slideshow->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="px-6 py-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  placeholder="Masukkan deskripsi slideshow (opsional)">{{ old('description', $slideshow->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if($slideshow->image)
                        <div>
                            ...
                            <div class="flex items-start space-x-4">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm text-gray-600">File saat ini: {{ basename($slideshow->image) }}</p>
                    ...
                    <div>
                        <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                            Gambar Slideshow {{ $slideshow->image ? '' : '<span class="text-red-500">*</span>' }}
                        </label>
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                        <span>{{ $slideshow->image ? 'Upload gambar baru' : 'Upload gambar' }}</span>
                        ...
                        <div id="image-preview" class="mt-3 hidden">
                            ...
                            <button type="button" id="remove-image" class="mt-2 text-sm text-red-600 hover:text-red-500">Hapus gambar baru</button>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-600">{{ $slideshow->created_at->format('d M Y H:i') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Terakhir Diperbarui</label>
                            <p class="mt-1 text-sm text-gray-600">{{ $slideshow->updated_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>
                </div>
