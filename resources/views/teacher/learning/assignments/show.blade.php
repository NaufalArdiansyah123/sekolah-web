                ...
                <div class="rounded-xl shadow-sm border overflow-hidden mb-8" style="background: var(--bg-secondary, #ffffff); border-color: var(--border-color, #e5e7eb);">
                    ...
                    <div class="p-6 border-b" style="border-color: var(--border-color, #e5e7eb);">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            @endphp
                            <span class="px-3 py-1 rounded-full text-sm font-medium" style="background: {{ $statusBg }}; color: {{ $statusText }};">
                                {{ ucfirst($assignment->status) }}
                            </span>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                            <div class="flex items-center" style="color: var(--text-secondary, #6b7280);">
                            ...
                            <div class="flex items-center" style="color: {{ $assignment->due_date < now() ? '#dc2626' : 'var(--text-secondary, #6b7280)' }};">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Description -->
                    <div class="p-6">
                        <h3 class="text-lg font-semibold mb-4" style="color: var(--text-primary, #111827);">Deskripsi Tugas</h3>
                        <div class="prose max-w-none mb-6" style="color: var(--text-primary, #111827);">
                            {!! nl2br(e($assignment->description)) !!}
                        </div>
                        
                        @if($assignment->instructions)
                            <div class="mb-6">
                                <h4 class="text-md font-semibold mb-3" style="color: var(--text-primary, #111827);">Instruksi</h4>
                                <div class="border rounded-lg p-4" style="background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.2);">
                                    <div class="prose max-w-none text-sm" style="color: var(--text-primary, #111827);">
                        ...
                        @if($assignment->attachment_path)
                            <div>
                                <h4 class="text-md font-semibold mb-3" style="color: var(--text-primary, #111827);">File Lampiran</h4>
                                <a href="{{ Storage::url($assignment->attachment_path) }}" 
                                   target="_blank"
                                   class="inline-flex items-center px-4 py-2 rounded-lg transition-colors" 
                                   style="background: var(--bg-tertiary, #f3f4f6); color: var(--text-primary, #374151);"
                                   onmouseover="this.style.background='var(--bg-quaternary, #e5e7eb)'"
                                   onmouseout="this.style.background='var(--bg-tertiary, #f3f4f6)'">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="rounded-xl shadow-sm border overflow-hidden" style="background: var(--bg-secondary, #ffffff); border-color: var(--border-color, #e5e7eb);">
                    <div class="p-6 border-b" style="border-color: var(--border-color, #e5e7eb);">
                        <div class="flex items-center justify-between">
                            <h2 class="text-xl font-semibold" style="color: var(--text-primary, #111827);">Ringkasan Pengumpulan</h2>
                            <a href="{{ route('teacher.assignments.submissions', $assignment->id) }}" 
                               class="font-medium" style="color: #2563eb;"
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium" style="color: var(--text-primary, #374151);">Progress Pengumpulan</span>
                                <span class="text-sm" style="color: var(--text-secondary, #6b7280);">
                                    {{ $submissionStats['submitted'] }}/{{ $submissionStats['total_students'] }} siswa
                                </span>
                            </div>
                            <div class="w-full rounded-full h-3" style="background: var(--bg-tertiary, #e5e7eb);">
                                <div class="bg-gradient-to-r from-blue-500 to-green-500 h-3 rounded-full transition-all duration-500" 
                                     style="width: {{ $submissionStats['submission_percentage'] }}%"></div>
                            </div>
                            <div class="flex justify-between text-xs mt-1" style="color: var(--text-secondary, #6b7280);">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium" style="color: var(--text-primary, #374151);">Progress Penilaian</span>
                                <span class="text-sm" style="color: var(--text-secondary, #6b7280);">
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            <div class="text-center p-4 rounded-lg" style="background: rgba(59, 130, 246, 0.1);">
                                <div class="text-2xl font-bold" style="color: #2563eb;">{{ $submissionStats['submitted'] }}</div>
                                <div class="text-sm" style="color: var(--text-secondary, #6b7280);">Terkumpul</div>
                            </div>
                            <div class="text-center p-4 rounded-lg" style="background: rgba(16, 185, 129, 0.1);">
                                <div class="text-2xl font-bold" style="color: #059669;">{{ $submissionStats['graded'] }}</div>
                                <div class="text-sm" style="color: var(--text-secondary, #6b7280);">Dinilai</div>
                            </div>
                            <div class="text-center p-4 rounded-lg" style="background: rgba(245, 158, 11, 0.1);">
                                <div class="text-2xl font-bold" style="color: #d97706;">{{ $submissionStats['ungraded'] }}</div>
                                <div class="text-sm" style="color: var(--text-secondary, #6b7280);">Belum Dinilai</div>
                            </div>
                            <div class="text-center p-4 rounded-lg" style="background: rgba(139, 92, 246, 0.1);">
                                <div class="text-2xl font-bold" style="color: #7c3aed;">{{ round($submissionStats['average_score'], 1) }}</div>
                                <div class="text-sm" style="color: var(--text-secondary, #6b7280);">Rata-rata</div>
                            </div>
                        </div>
                        ...
                        @if($recentSubmissions->count() > 0)
                            <div class="mt-6">
                                <h3 class="text-lg font-semibold mb-4" style="color: var(--text-primary, #111827);">Pengumpulan Terbaru</h3>
                                <div class="space-y-3">
                                    @foreach($recentSubmissions as $submission)
                                        <div class="flex items-center justify-between p-3 rounded-lg" style="background: var(--bg-tertiary, #f9fafb);">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                @if($submission->graded_at)
                                                    <span class="text-xs px-2 py-1 rounded" style="background: rgba(16, 185, 129, 0.1); color: #059669;">
                                                        {{ $submission->score }}/{{ $assignment->max_score }}
                                                    </span>
                                                @else
                                                    <span class="text-xs px-2 py-1 rounded" style="background: rgba(245, 158, 11, 0.1); color: #d97706;">
                                                        Belum dinilai
                                                    </span>
                                                @endif
                                                <a href="{{ route('teacher.assignments.grade', [$assignment->id, $submission->id]) }}" 
                                                   class="text-sm" style="color: #2563eb;"
                                                   onmouseover="this.style.color='#1d4ed8'"
                                                   onmouseout="this.style.color='#2563eb'">
            ...
            <div class="lg:col-span-1">
                ...
                <div class="rounded-xl shadow-sm border p-6 mb-6" style="background: var(--bg-secondary, #ffffff); border-color: var(--border-color, #e5e7eb);">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <span class="text-sm" style="color: var(--text-secondary, #6b7280);">Total Siswa:</span>
                            <div class="font-medium" style="color: var(--text-primary, #111827);">{{ $submissionStats['total_students'] }} siswa</div>
                        </div>
                        
                        <div>
                            <span class="text-sm" style="color: var(--text-secondary, #6b7280);">Dibuat pada:</span>
                            <div class="font-medium" style="color: var(--text-primary, #111827);">{{ $assignment->created_at->format('d M Y') }}</div>
                        </div>
                        ...
                        @if($assignment->due_date < now())
                            <div class="p-3 border rounded-lg" style="background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.2);">
                                <div class="text-sm font-medium" style="color: #dc2626;">
                                    Deadline terlewat {{ $assignment->due_date->diffForHumans() }}
                                </div>
                            </div>
                        @else
                            <div class="p-3 border rounded-lg" style="background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.2);">
                                <div class="text-sm font-medium" style="color: #2563eb;">
                ...
                <div class="rounded-xl shadow-sm border p-6" style="background: var(--bg-secondary, #ffffff); border-color: var(--border-color, #e5e7eb);">
                    <h3 class="text-lg font-semibold mb-4" style="color: var(--text-primary, #111827);">Aksi Cepat</h3>
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md" style="background: var(--bg-secondary, #ffffff); border-color: var(--border-color, #e5e7eb);">
        <div class="mt-3 text-center">
            <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full" style="background: rgba(239, 68, 68, 0.1);">
            ...
            <h3 class="text-lg leading-6 font-medium" style="color: var(--text-primary, #111827);">Hapus Tugas</h3>
            <div class="mt-2 px-7 py-3">
                <p class="text-sm" style="color: var(--text-secondary, #6b7280);">
