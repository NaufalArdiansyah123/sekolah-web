            ...
            <div class="flex items-center justify-between">
                ...
                <div class="text-center">
                    <div id="timer" class="text-3xl font-bold text-red-600 dark:text-red-400">
                        <span id="minutes">{{ $attempt->remaining_time }}</span>:<span id="seconds">00</span>
                    </div>
                            ...
                            @if($question->type === 'multiple_choice')
                                @foreach($question->formatted_options as $key => $option)
                                    <label class="flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                        ...
                                        <span class="ml-3 text-gray-900 dark:text-white">{{ $option }}</span>
                                    </label>
                                @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    // Timer functionality
    let timeLeft = {{ $attempt->remaining_time }} * 60; // Convert to seconds
    ...
    inputs.forEach(input => {
        input.addEventListener('change', function() {
            ...
            localStorage.setItem('quiz_' + {{ $attempt->id }} + '_' + this.name, this.value);
        });
    });
    
    // Restore saved answers
    inputs.forEach(input => {
        const saved = localStorage.getItem('quiz_' + {{ $attempt->id }} + '_' + input.name);
        if (saved) {
            if (input.type === 'radio') {
    ...
    form.addEventListener('submit', function() {
        inputs.forEach(input => {
            localStorage.removeItem('quiz_' + {{ $attempt->id }} + '_' + input.name);
        });
    });
});
</script>
