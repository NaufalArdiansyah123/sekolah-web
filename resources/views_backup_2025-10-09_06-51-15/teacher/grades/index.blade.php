...
<div class="grades-container">
            ...
            <div class="header-left">
                ...
                <div class="header-stats">
                    <div class="header-stat">
                        <div class="header-stat-value">{{ $stats['total_grades'] }}</div>
                        <div class="header-stat-label">Total Nilai</div>
                    </div>
                    <div class="header-stat">
                        <div class="header-stat-value">{{ number_format($stats['average_score'], 1) }}</div>
                        <div class="header-stat-label">Rata-rata</div>
                    </div>
                    <div class="header-stat">
                        <div class="header-stat-value">{{ $stats['subjects_count'] }}</div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card semester">
            ...
            <div class="stat-value">{{ $stats['this_semester'] }}</div>
    ...
    <div class="grades-table-container">
        <div class="table-header">
            ...
            <div class="table-actions">
                ...
                <a href="{{ route('teacher.grades.report') }}" class="btn btn-secondary btn-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    Laporan
                </a>
                <a href="{{ route('teacher.grades.recap') }}" class="btn btn-secondary btn-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="grades-table" id="gradesTable">
                ...
                <tbody>
                    @foreach($grades as $grade)
                        <tr data-grade-id="{{ $grade->id }}" onclick="toggleRowSelection(this, event)">
                            <td>
                                <div class="checkbox-wrapper">
                                    <input type="checkbox" class="row-checkbox" value="{{ $grade->id }}" onchange="updateBulkActions()">
                                </div>
                            </td>
                            <td>
                                <div class="student-info">
                                    ...
                                    <div class="student-details">
                                        <div class="student-name">{{ $grade->student->name }}</div>
                                        <div class="student-class">Kelas {{ $grade->student->class ?? 'N/A' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <span class="type-badge {{ $grade->type }}">
                                    {{ ucfirst($grade->type) }}
                                </span>
                            </td>
                            <td>
                                <div class="score-display">
                                    <div class="score-main">{{ $grade->score }}</div>
                                    <div class="score-max">/ {{ $grade->max_score }}</div>
                                    <div class="score-percentage">{{ number_format($grade->percentage, 1) }}%</div>
                                </div>
                            </td>
                            <td>
                                @php
                                ...
                                @endphp
                                <span class="grade-badge {{ $badgeClass }}" title="{{ number_format($percentage, 1) }}%">
                                    {{ $grade->letter_grade }}
                                </span>
                            </td>
                            <td>
                                <div class="date-display">
                                    <div class="date-main">{{ $grade->created_at->format('d M Y') }}</div>
                                    <div class="date-time">{{ $grade->created_at->format('H:i') }}</div>
                                </div>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <a href="{{ route('teacher.grades.show', $grade->id) }}" 
                                    ...
                                    <button onclick="deleteGrade({{ $grade->id }}, event)" 
...
function exportToExcel() {
    ...
    link.href = `{{ route('teacher.grades.index') }}?${params.toString()}`;
...
function bulkExport() {
    ...
    form.method = 'POST';
    form.action = '{{ route("teacher.grades.index") }}';
...
function downloadTemplate() {
    const link = document.createElement('a');
    link.href = '{{ route("teacher.grades.index") }}?download=template';
...
document.getElementById('importForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("teacher.grades.index") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function deleteGrade(id, event) {
    ...
    fetch(`{{ route('teacher.grades.index') }}/${id}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'X-Requested-With': 'XMLHttpRequest'
